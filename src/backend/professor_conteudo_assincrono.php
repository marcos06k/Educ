...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['arquivo'])) {
        ...
        if ($file["error"] === UPLOAD_ERR_OK) {
            ...
            $pastaDir = 'C:/xampp/htdocs/backend-pi/src/files/conteudo_assincrono/';  
            ...
            if (move_uploaded_file($file["tmp_name"], $uploadPath)) {
                ...
                $query_upload_url = mysqli_query($banco, "insert into conteudo_assincrono values(null, '$arquivo_pasta', '$nome');");
                if($query_upload_url){
                    echo "Atividade registrada com sucesso!";
                    // pegar o id da atividade inserida no banco 
                    $id_conteudo_assincrono = $banco->insert_id;
                    $query_conteudo_assincrono = mysqli_query($banco, "insert into aula_conteudo_assincrono values(null, '$id_aula','$id_conteudo_assincrono');");
                } else {
